...
<ul>
  ...
  <li>(22/05/09)
    <ol>
      <li>header의 nav의 각 리스트를 클릭했을 때 해당 섹션으로 스크롤 이벤트를 통해 넘어가도록 구현했다. header의 내용을 따로 파일로 만든 뒤 로드를 해놓아서 처음에는 이벤트함수를 넣어도 적용이 안됐다. 방법을 찾다가 load된 곳에 콜백함수로 이벤트를 넣어주니 적용이 되었다.</li>
      <li>fna섹션에 toggleClass가 아닌 아코디언 slide를 활용하여 구현했다. (여전히 p태그만 움직이는 중)</li>
    </ol>
  </li>
</ul>
